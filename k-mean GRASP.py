...
def clustering(centre_array, df):
# Calculate the distance of each customer to the different depots
    
    for nodes in range(len(df)):
        min_distance = np.inf
        for centre in range(len(centre_array)):
            distance = dist(centre_array[centre],  (df.iloc[nodes, 0], df.iloc[nodes, 1] )  )
            if distance < min_distance:
                # Assosiate the node to the nearest centre
                min_distance = cp.deepcopy(distance)
                df.iloc[nodes, -1] = centre
                
    return df
                
def k_mean(df, max_iteration):
    centre_array = []
    
    # First randomly generate 3 points to use as centre
    for k in range(cluster):
         value = (rd.randrange(min_x, max_x), rd.randrange(min_y, max_y))
         centre_array.append(value)
    
    df["cluster"] = 0
    i = 0
    
    while i < max_iteration:
        # Use the centre of every new cluster to construct the next cluster
        old_centre = cp.deepcopy(centre_array)
        
        df = clustering(centre_array, df)
        
        for centre in range(cluster):
            centre_array[centre] = (df.loc[df["cluster"] == centre,"XCOORD"].mean(),
                                    df.loc[df["cluster"] == centre,"YCOORD"].mean())
        
        increment = np.abs((np.array(old_centre) - np.array(centre_array)).mean())
    ...
    return df, centre_array

df_cluster, centre_array = k_mean(df_customer, 1000)

# Assign the customer cluster to the nearest depot in a dictionary
cluster_dict = {}
...
for k in range(cluster):
     ...
     for j in range(cluster):
         distance = dist(centre_array[k], df_depots.iloc[j, 0:2])
         distance_from_centre.append(distance)
         
     min_index = distance_from_centre.index(min(distance_from_centre))
     key = "Depot{0}".format(min_index)
     value = df_cluster.loc[df_cluster["cluster"] == k]
...
for i in range(cluster):
    ...
    cluster_dict[key] = pd.concat([new_row, cluster_dict[key]])
    value = distmatrix(cluster_dict[key])
        ...
        # Randomly choose a centre and k nearest neighbour to be removed from service
            ...
            for route in range(len(routes)):
                for node in nearest_neighbour:
                    if node != routes[route][0] and node != routes[route][-1]:
...
for i in range(cluster):
    ...
    for routes in range(len(output2[1])):
        for nodes in range(len(output2[1][routes])):
            output2[1][routes][nodes] = cluster_dict["Depot{0}".format(i)].index[output2[1][routes][nodes]]
    
    unified_routes += output2[1]
    unified_cost += output2[0]
    results.append(output2)

unified_distance_matrix = distmatrix(df)
    
final_solution_cost, final_solution = simulated_annealing(unified_routes, unified_cost, 500, 1000, 0.95, unified_distance_matrix)
